...
import type { CartItem, Listing, Notification, Order, UserProfile } from '@/types';
...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'ADD_TO_CART': {
      const existing = state.cart.find((item) => item.listing.id === action.payload.id);
      if (existing) {
        return {
          ...state,
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<AppContextValue>(() => {
    ...
    const unreadNotificationCount = state.notifications.filter((n) => !n.is_read).length;
    const userListings = state.listings.filter((l) => l.seller_id === state.user?.id);
